...
double calcMSE(Trial *trial, int num);    // MSEを計算する
...
//      ここで、推定用関数はestimateLinearFuncで定められている
//------------------------------------------------------------
double calcMSE(Trial *trial, int num) {
    int i;           // カウンタ変数
    double sum = 0;  // 各データについての誤差の二乗の総和
    
    /*---誤差の二乗の総和を求める---*/
    for (i = 0;i < num;i++) {
        sum += pow((double)calcError(trial[i]), 2.0);
    }
