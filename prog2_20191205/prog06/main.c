...
//その他の関数
int convertToIndex(char eqScale);                           // 震度を数字に変換する
...
int main(int argc, const char * argv[]) {
    ...
    char eqScale;                //------+
    char eqScales[9] = {'1', '2', '3', '4', 'A', 'B', 'C', 'D', '7'};  // 震度を表す文字の集合
    int cnt[9] = {};                                                   // 各震度ごとの地震の回数
                                                                       // 添字は震度1,~,4,A,~,B,7に対して0~8に対応
    ...
    /*---各震度の地震の回数をカウント---*/
    for(i = 0;i < num;i++) {
        cnt[convertToIndex(getEqScale(eq[i]))]++;
    }
...
//その他の関数
//------------------------------------------------------
//機能:震度を表す文字から配列の添字に変化する
//引数 char eqScale:変換対象の震度を表す文字
//返り値:eqScaleが'1'~'4'のとき0~3、
//      eqScaleが'A'~'D'のとき4~7
//      eqScaleが'7'のとき8を返す
//------------------------------------------------------
int convertToIndex(char eqScale) {
    char eqScales[9] = {'1', '2', '3', '4', 'A', 'B', 'C', 'D', '7'};  // 震度を表す文字の集合
    int i;                                                             // カウンタ変数
    int ret = 0;                                                       // 返り値
    
    /*---eqScaleとeqScales[i]が一致する添字を返す---*/
    for(i = 0;i  <9;i++) {
        if(eqScale == eqScales[i]) {
            ret = i;
        }
    }
