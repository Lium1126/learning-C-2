...
//操作関数
void printEarthQuake(EarthQuake eq);                                        // 地震情報を1つ表示する
void printEqOfSpecifiedDate(EarthQuake *eq, int num, int month, int date);  // 特定の日付の地震情報を表示する
...
int main(int argc, const char * argv[]) {
    ...
    /*---eqの内容を表示---*/
    printEqOfSpecifiedDate(eq, num, 3, 11);
...
//------------------------------------------------------
//機能:num個の地震情報eqのうち、特定の日付のものを標準出力に表示する
//引数 EarthQuake *eq:表示する対象の地震情報の配列
//    int num       :eqのサイズ
//    int month     :---+
//    int date      :---+--地震情報を表示する日付
//返り値:なし
//------------------------------------------------------
void printEqOfSpecifiedDate(EarthQuake *eq, int num, int month, int date) {
    int i;   // カウンタ変数
    
    /*---全データの中から特定の日付のものだけ表示---*/
    for(i = 0;i < num;i++) {
        if(getMonth(eq[i]) == month && getDate(eq[i]) == date) {
            printEarthQuake(eq[i]);
        }
    }
