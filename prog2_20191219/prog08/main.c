...
int main(int argc, const char * argv[]) {
    ...
    // 1-NNを行い、推定結果と正誤を出力すると同時に、混同行列を作成
    correct_cnt = 0;
    for (i = 0;i < size_test;i++) {
        ...
        makeConfusionMatrix(confusion_matrix, array_test[i], estimated_result);
    ...
    /*---推定が正解であった割合を表示---*/
    printf("-----\n");
    printf("推定が正解する割合: %.3f\n", (double)correct_cnt / (double)size_test);
    ...
    for (i = 0;i < 3;i++) {
        for (j = 0;j < 3;j++) {
            if (num_class[i] != 0) {
                confusion_matrix[i][j] /= (double)num_class[i];
            }
        }
    }
