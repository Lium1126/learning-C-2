...
double linearFunction(double a, int x);
double calcMSE(Trial *trial, int n, double a);
...
int main(int argc, const char * argv[]) {
    ...
    printf("推定: distance = %lf * speed\n", a);
    
    /*---dispTrialを用いて配列の内容と推定との誤差を表示---*/
    for (i = 0;i < n;i++) {
        dispTrial(trial[i]);
        printf(", 推定との誤差: %lf\n", (double)getDistance(trial[i]) - linearFunction(a, getSpeed(trial[i])));
    }
...
double calcMSE(Trial *trial, int n, double a) {
    ...
    for (i = 0;i < n;i++) {
        sum += pow((double)((double)getDistance(trial[i]) - linearFunction(a, getSpeed(trial[i]))), 2.0);
    }
