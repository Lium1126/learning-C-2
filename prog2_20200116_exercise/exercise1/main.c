...
typedef MCell MList;      // 意味合いによって使い分け
...
void fprint_mlist(FILE *out_fp, MList *head);                               // リストの内容をファイルに出力する
...
int main(int argc, const char * argv[]) {
    ...
    FILE *fp;                           // 入力ファイルおよび出力ファイルのファイルポインタ
    ...
    /*---入力ファイルから入力を受け付け、リストに格納する---*/
    while (fgets(input_line, LINE_LEN, fp) != NULL) {
        sscanf(input_line, "%[^,],%lf,%lf", input_name, &input_height, &input_weight);
...
//------------------------------------------------------------------
//機能:リストの内容をファイルに出力する
//引数 FILE *out_fp:出力ファイルのファイルポインタ
//    MList *head :要素を出力するリストを指すポインタ
//返り値:なし
//------------------------------------------------------------------
void fprint_mlist(FILE *out_fp, MList *head) {
    if (isEmptyList(head)) {
        return;
    }
    
    fprintf(out_fp, "%s,%.1f,%.1f\n", getName(head), getHeight(*head), getWeight(*head));
    fprint_mlist(out_fp, getNextMCell(*head));
}
